...
def update_chromedriver_if_needed(version_path):
    ...
    logging.info(f"Chrome Browser Version: {browser_version}")

    # Read the current ChromeDriver version from file
    try:
        with open(version_path, "r") as file:
            driver_version = file.read().strip()
        logging.info(f"Current ChromeDriver Version: {driver_version}")
    except FileNotFoundError:
        logging.warning(f"Version file not found at {version_path}. Assuming no driver installed.")
        driver_version = None

    # Update ChromeDriver if versions do not match
    if browser_version != driver_version:
        logging.info("Updating ChromeDriver...")
        ...
        if compatible_driver:
            logging.info(f"Found compatible driver version: {compatible_driver['version']}")
            ...
            if compatible_driver['version'] == driver_version:
                logging.info(f"ChromeDriver is already up to date with version: {driver_version}")
            else:
            ...
            logging.error("No compatible driver version found.")
    else:
...
def download_chromedriver(url, driver_path):
    ...
    try:
        ...
        logging.warning("Unable to delete 'chromedriver.zip' - it may be in use by another process.")
